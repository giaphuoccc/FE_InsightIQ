...
export class TenantAccountListComponent implements OnInit {
  ...
  loadTenantWithDetails(): void {
    ...
    this.tenantService.getAllTenant().pipe( //Gọi API lấy danh sách Tenant cơ bản
      mergeMap((basicTenantFromApi: any[]) => {
        // basicTenantFromApi là mảng các Tenant nhưng chưa có email với phone number

        if (!basicTenantFromApi || basicTenantFromApi.length === 0) { // Trường hợp k có Tenant nào
        ...
        const tenantDetailObservables = basicTenantFromApi.filter(tenantApiData => tenantApiData.status === 'APPROVED' || tenantApiData.status === 'DISABLED' ).map(tenantApiData => {
          ...
          return this.tenantService.getUserDetail(currentUserId).pipe(
            map((userDetailsFromApi: any) => {
              return {
                ...
                isBlocked: tenantApiData.status === 'DISABLED' || tenantApiData.status === 'REJECTED',
  ...
  confirmAction(): void {
    ...
    if(!this.selectedTenant){ // Nếu không có Tenant nào thì đóng modal
      this.closeConfirm();
      return;
    ...
    if (this.actionType === 'block'){ //Người dùng muốn disable Tenant
      newApiStatusValue = 'DISABLED';
      newFrontendIsBlockedValue = true;

    } else if (this.actionType === 'unblock'){ //SA muốn active Tenant
    ...
    this.tenantService.updateTenantStatus(this.selectedTenant.id, tenantDto)
    .subscribe({
      next: (response) =>{
