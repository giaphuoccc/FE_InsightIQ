...
import {
  ...
  HttpClientModule,
  HttpHeaders,
} from '@angular/common/http'; // Import HttpClient, HttpErrorResponse, HttpClientModule, HttpHeaders
...
export class NavbarComponent {
  ...
  onLogoClick(): void {
    ...
    const tenantRequest = this.http
      .get<UserInfoResponse>(tenantApiUrl, httpOptions)
      .pipe(
        // <= SỬ DỤNG httpOptions
        catchError((error: HttpErrorResponse) => {
          console.warn(
            'Tenant API call failed or user is not a tenant:',
            error.message
          );
          return of(null);
        })
    ...
    const superAdminRequest = this.http
      .get<UserInfoResponse>(superAdminApiUrl, httpOptions)
      .pipe(
        // <= SỬ DỤNG httpOptions
        catchError((error: HttpErrorResponse) => {
          console.warn(
            'SuperAdmin API call failed or user is not a superadmin:',
            error.message
          );
          return of(null);
        })
    ...
    forkJoin([tenantRequest, superAdminRequest]).subscribe(
      ([tenantResponse, superAdminResponse]) => {
        ...
        if (tenantResponse && tenantResponse.role === 'TENANT') {
          ...
          superAdminResponse &&
          superAdminResponse.role === 'SUPERADMIN'
        ) {
          console.log(
            'User is SUPERADMIN, navigating to superadmin dashboard.'
          );
        ...
        if (!navigated) {
          console.warn(
            'Could not determine user role or role is not recognized from API responses.'
          );
